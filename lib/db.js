...
// TODO: I NEED TO DO A METHOD TO SORT KEYS BEFORE INSERTING DATA OTHERWISE IF ORDER IS DIFFERENT IT WILL THROW AN ERROR
...
async function updateUserWebsite(uid, wid, data) {
  const params = [...data, uid, wid];
  const { rows } = await dbInstance.query(
    "update websites set name = $1, url = $2, shared = $3 where user_id = $4 and id = $5 returning *",
    params
  );
...
// TODO: delete also website events.
async function deleteUserWebsite(uid, wid) {
  const params = [uid, wid];
  const { rowCount } = await dbInstance.query(
    "delete from websites where user_id = $1 and id = $2",
    params
  );
